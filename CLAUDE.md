...
- `libs/*` — pure TypeScript libraries, consumed by the main process only. Aliased as `@nightcoder/<name>` via both `tsconfig.base.json` `paths` and each lib's `package.json` name. Both must stay in sync or imports resolve in the editor but fail at runtime.
...
1. **Renderer → Main over IPC.** Channel names live in `libs/shared-types/src/ipc.ts` (`IpcChannels`, `IpcEvents`). The renderer calls `window.nightcoder.*` (typed as `IpcApi`), which is wired up in the preload script. All DTOs (`Story`, `RefinedPlan`, `RunEvent`, etc.) are declared here and re-validated with Zod schemas in `libs/shared-types/src/schemas.ts`. If you add a field, update both or runtime validation drops it.
2. **Main — planning.** `apps/desktop/src/main/index.ts` receives `Refine` / `Advise` IPC calls and delegates to `@nightcoder/ai-refiner`. `refine()` spawns the local `claude` CLI with `--output-format json --json-schema <RefinedPlanSchema-as-json-schema>` so the model returns a plan as structured output. `collectRepoContext()` reads `README`, `package.json`, and any `CLAUDE.md`/`AGENTS.md` from the target repo and prepends them to the user prompt.
3. **Main — orchestration.** `StartRun` delegates to `@nightcoder/orchestrator`'s `startPlanRun()`. It:
...
- **Local AI settings are mirrored into each worktree** (`libs/orchestrator/src/worktree.ts`): `.claude/settings.local.json`, `.claude/CLAUDE.local.md`, `.agents/settings.local.json`, `.env.local`. Set `NIGHTCODER_INHERIT_LOCAL_AI=0` to disable. These files are deleted on worktree cleanup.
