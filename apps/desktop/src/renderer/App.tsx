...
import { StoryCard, type StoryRuntime } from './components/StoryCard';
...
export default function App() {
  ...
  const handleCancelStory = async (storyIndex: number) => {
    if (!runId) return;
    await window.zibby.cancelStory({ runId, storyIndex });
  };
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        {plan && (
          <PlanView
            ...
            onCancelStory={handleCancelStory}
...
function PlanView({
  ...
  onRun: () => void;
  onCancel: () => void;
  onCancelStory: (storyIndex: number) => void;
        ...
        {plan.stories.map((story, i) => {
          ...
          return (
            <StoryCard
              ...
              onStop={running ? () => onCancelStory(i) : undefined}
            />
          );
        })}
      </div>
