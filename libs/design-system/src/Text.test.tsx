...
describe('Text', () => {
  ...
  it('maps size and weight to inline styles', () => {
    render(<Text size="xs" weight="semibold">x</Text>);
  ...
  it('maps tone tokens onto color', () => {
    ...
    expect(screen.getByText('x').style.color).toBe(rgb(defaultDarkTokens.color.accent.emerald));
  });
  ...
  it('forwards arbitrary props (className, role, title)', () => {
    render(<Text className="x" role="note" title="t">y</Text>);
  ...
  it('inline `style` overrides win over computed defaults', () => {
    render(<Text size="md" style={{ fontSize: '99px' }}>x</Text>);
